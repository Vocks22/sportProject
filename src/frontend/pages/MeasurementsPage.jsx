...
const MeasurementsPage = () => {
  ...
  const fetchMeasurements = async () => {
    try {
      const response = await fetch(`http://localhost:5000/api/users/${userId}/measurements?days=365&limit=100`);
      if (response.ok) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`http://localhost:5000/api/users/${userId}/measurements`, {
        method: 'POST',
